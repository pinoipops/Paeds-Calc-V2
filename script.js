...
/* ==================================================
   QUICK SHORTCUTS
   ================================================== */
const QUICK_SHORTCUTS = ["paracetamol", "ibuprofen", "augmentin", "amoxicillin", "salbutamolNeb"];
...
function buildShortcuts(){
  ...
  QUICK_SHORTCUTS.forEach(key=>{
    ...
    button.innerHTML=`
      <span class="shortcut-name">${escapeHtml(DATA[key].name)}</span>
      <button class="star-btn" type="button" aria-label="Toggle favourite">☆</button>
    `;
    button.addEventListener("click",(event)=>{
      if(event.target.closest(".star-btn")) return;
      selectMedication(key);
    });
    button.querySelector(".star-btn").addEventListener("click",(event)=>{
...
function refreshShortcutState(){
  shortcutGrid
    ...
    .forEach(button=>{
      ...
      button.classList.toggle("is-selected", key===medicationSelect.value);
      button.classList.toggle("is-favourite", favourites.includes(key));
    });
...
/* ==================================================
   FIELD VISIBILITY LOGIC (AGE & WEIGHT)
   ================================================== */
function updateFieldVisibility(drug){
  const needsAge = drug.dosing==="age" || drug.dosing==="ageWeight";
  const needsWeight = drug.dosing.toLowerCase().includes("weight") || drug.dosing==="weightBand";

  ageField.classList.toggle("hidden", !needsAge);
  weightField.classList.toggle("hidden", !needsWeight);

  if(needsAge){
    ageHelp.textContent = drug.dosing==="ageWeight"
...
function matchesAge(rule,age){
  if(rule.ageMin!==undefined && age<rule.ageMin) return false;
  if(rule.ageMax!==undefined && age>rule.ageMax) return false;
  return true;
}
...
function calculateDose(){
  ...
  if(needsAge && (!Number.isFinite(age)||age<0)){
  ...
  if(drug.dosing==="age"){
    const matching = drug.regimens.find(candidate=>matchesAge(candidate,age));
    if(matching){
      rule=matching;
  ...
  if(drug.dosing==="ageWeight"){
    const matching = drug.regimens.find(candidate => matchesAge(candidate,age) && matchesWeight(candidate,weight));
    if(matching){
      rule=matching;
      regimenSelect.value = drug.regimens.indexOf(matching);
    }
  ...
  if(rule.fixedMl!==undefined){
    ...
    calculationText.textContent="Use the selected age-specific dose.";
    showNotes(drug,rule);
  ...
  /* RANGE mL */
  if(rule.rangeMl){
    doseValue.textContent = `${formatRange(rule.rangeMl)} mL`;
    volumeValue.textContent="";
    frequencyValue.textContent=rule.frequency||"—";
    frequencySub.textContent=rule.frequencySub||"";
    calculationText.textContent="Use an appropriate dose within the displayed range.";
    showNotes(drug,rule);
    resultCard.classList.remove("hidden");
    return;
  }
